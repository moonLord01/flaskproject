...
app.config['SECRET_KEY'] = 'jraduevoid'
db = SQLAlchemy(app)
admin = Admin(app)
...
class Category(db.Model):
    ...
    articles = db.relation('Article', backref='category_name')
...
@app.route('/news')
def news():
    category = Category.query.filter_by(name='Tazelikler').first()
    articles = Article.query.filter_by(category_id=category.id)
...
def new_post():
    if request.method == 'POST':
        ...
        try:
            ...
            return f'Voznikla oshibka -> {error}'
    else:
...
def delete_post(article_id):
    ...
    try:
        ...
        return redirect(url_for('index'))
            ...
            return f'Voznikla oshibka pri udalenii-> {error}'
